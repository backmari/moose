  ...
  if (effective_trial_stress != 0.0)
  {
           ...
           !converged(residual, reference_residual))
    {
      ...
      else
      {
        ...
        bool bounded = false;
        // Check to see whether trial scalar_increment is outside the bounds, and set it to a point within the bounds if it is
            ...
            scalar_old + scalar_increment <= scalar_lower_bound)
        {
              ...
              scalar_lower_bound != 0.0)
          {
            ...
            Real frac = 0.5;
            scalar_increment = (1.0 - frac) * scalar_lower_bound
                               + frac * scalar_upper_bound
                               - scalar_old;
            //scalar_increment = (scalar_upper_bound + scalar_lower_bound) / 2.0 - scalar_old;
            modified_increment = true;
            bounded = true;
            if (_output_iteration_info == true || _output_iteration_info_on_error == true)
              iter_output << "  Trial scalar_increment exceeded bounds.  Setting between lower/upper bounds. frac: " << frac << std::endl;
          }
        }
                                  ...
                                  bool bounded)
{
       ...
       (scalar_upper_bound - scalar_lower_bound) / (PETSC_MACHINE_EPSILON * scalar_lower_bound) <= 10.0))
  {
    ...
    if (_output_iteration_info == true || _output_iteration_info_on_error == true)
      iter_output << "  Reset lower/upper bounds because they're too close together" << std::endl;
           ...
           (std::abs(residual) > 0.9*std::abs(resid_lower_bound) ||
            std::abs(residual) > 0.9*std::abs(resid_upper_bound)))
  {
    ...
    scalar_upper_bound = std::numeric_limits<Real>::max();
    resid_upper_bound = 0.0;
    iter_output << "  Reset lower/upper bounds because residual didn't decrease enough" << std::endl;
  }
}
